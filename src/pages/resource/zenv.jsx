...
//引入请求登录函数 20200107
import {reqPlexes, reqUpdatePlex, reqAddPlex, reqDelPlex} from '../../api/index'


//20210719
import {reqEnvs} from '../../api/index'

import {reqPlexLpars} from '../../api/index'
...
//在form.jsx文件定义form 第一个字母必惜大写
import ShowLparForm from './showlpar-form'
...
export default class Zenv extends Component {
    ...
    */
        ...
        },
        {
          ...
          render: (plex) => (
            <span>
              ...
              <LinkButton onClick={() => {this.delplex(plex)}}>删除Plex</LinkButton>
            </span>
          ),
        },
      ]
    ...
    showLpars = async (plex) => {
      ...
      //注意此处与接受请求的api格式保持一致
      const result = await reqPlexLpars({plexname: plex.plexname})

      
      if(result.status===0) {
        //取出数组
        const plexlpars = result.data
        this.setState({
          plexlpars
        })
      }
  ...
  //
  delplex = async (plexname) => {

    console.log("delplex")
    const result = await reqDelPlex(plexname)
    if (result.status === 0){
      // 3 重新显示列表
      this.getPlexes()
      alert(result.msg)
  ...
  */
  ...
  addPlex =  () => {
    ...
    this.addform.current.validateFields().then(async (value) =>
      // (value) => {console.log("value :",value)
      {
        ...
        //准备数据  this.form ??
        // const plexname = this.addform.current.getFieldsValue().plexname
        const plexname = value.plexname //两种方法都可以取数据
        const use = this.addform.current.getFieldsValue().plexuse
        const total = this.addform.current.getFieldsValue().plextotal
        const used = this.addform.current.getFieldsValue().plexused
        const free = this.addform.current.getFieldsValue().plexfree
        ...
        // 2 发请求更新分类
        console.log(plex)
        const result = await reqAddPlex(plex)
        if (result.status === 0){
          // 3 重新显示列表
          this.getPlexes()
        }else{
  ...
  */
  ...
  render() {
      ...
      // 读取状态数据
      const {plexes, loading, showStatus, envs, plexlpars} = this.state
          ...
          <div>
              <Card title={title} extra={extra}>
                ...
                pagination={{defaultPageSize: 5, showQuickJumper: true}}
                  ...
                  <AddFormPlex envs = {envs} setFormadd={(addform)=>{this.addform=addform}}/>   {/* //传参给子组件 */}
                </Modal>
                ...
                <Modal title={plex + "lpar列表"} 
                  destroyOnClose = 'true'  //关闭时候销毁以便打开时重新载入数据 如果 modal 包含 form 需要在 form设定  preserve = {false}
                  visible={showStatus===3} 
                  onOk={this.handleCancel} 
                  onCancel={this.handleCancel}
                >
                <ShowLparForm plex = {plex} plexlpars={ plexlpars } setlparForm={(setlparForm)=>{this.setlparForm=setlparForm}}/>  
                </Modal>
              </Card>
          </div>
      )
